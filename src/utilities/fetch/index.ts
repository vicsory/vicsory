...
export const getAllPosts = async (page = "1", category?: string) => {
  const url = new URL(`${HOST_URL}/api/posts/all`);
...
export const getRelatedJobs = async () => {
  const response = await fetch(`${HOST_URL}/api/jobs/related`, {
    next: {
      revalidate: 0,
    },
  });
...
export const getUserJobs = async (username: string) => {
  const response = await fetch(`${HOST_URL}/api/jobs/${username}`, {
    next: {
      revalidate: 0,
    },
  });
...
export const getUserJob = async (jobId: string, jobAuthor: string) => {
  const response = await fetch(`${HOST_URL}/api/jobs/${jobAuthor}/${jobId}`, {
    next: {
      revalidate: 0,
    },
  });
...
export const createJob = async (job: string) => {
  const response = await fetch(`${HOST_URL}/api/posts/create`, {
    method: "JOB",
    headers: {
      "Content-Type": "application/json",
...
export const addStory = async ({ photoUrl }: { photoUrl: string }) => {
  console.log("addStory payload:", { photoUrl });
  const response = await fetch(`${HOST_URL}/api/stories/create`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  const json = await response.json();
  console.log("addStory response:", json);
  if (!json.success) throw new Error(json.message || "Something went wrong.");
  return json;
};
