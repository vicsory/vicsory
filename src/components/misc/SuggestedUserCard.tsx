...
export default function SuggestedUserCard() {
    ...
    const handleRemoveUser = (username: string) => {
        setVisibleUsers((prevUsers) => prevUsers.filter((user) => user.username !== username));
    };
    ...
    return (
        <div className="bg-[var(--background-primary)] border-t border-b border-solid border-[var(--border-color)] shadow-sm overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-3 border-b border-solid border-[var(--border-color)]">
                <h1 className="text-xl font-semibold text-[var(--active-mode)]">
                    Suggested for You
                </h1>
                <button
                    className="text-[var(--active-mode)] hover:bg-[var(--hover)] p-1 rounded-full transition-colors duration-150"
                    onClick={handleClose}
            ...
            {visibleUsers.length > 0 ? (
                <div className="flex overflow-x-auto gap-6 px-4 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {visibleUsers.map((user) => (
                        ...
                        >
                            <User user={user} onRemove={handleRemoveUser} />
                        </div>
                    ))}
                </div>
            ) : (
                <div className="px-4 py-4 text-sm text-[var(--text-2)] text-center">
