...
export default function NewPost({ token, handleSubmit }: NewPostProps) {
  ...
  const validationSchema = yup.object({
    text: yup.string().max(280, "Maximum 280 characters.").required("Post cannot be empty."),
  });
  ...
  return (
    <div className="flex gap-4 p-4 px-4 bg-[var(--background-primary)] border-y border-solid border-[var(--border)] rounded-2xl">
      {/* Avatar */}
      <Image
      ...
      <form className="flex-1 flex flex-col gap-3" onSubmit={formik.handleSubmit}>
        <textarea
          name="text"
          placeholder="“Post your latest update…"
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            <button
              ...
              className="p-2 rounded-full hover:bg-[var(--background-secondary)]"
            >
          ...
          <button
            ...
            className={`px-4 py-1 rounded-full ${
              formik.isValid
                ? "text-[var(--text)] bg-[var(--background-secondary)] hover:bg-[var(--background-primary)] border border-solid border-[var(--border)] transition"
